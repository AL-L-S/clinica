...
class internacao_model extends BaseModel {
    ...
    function listapacienteunidadeteste($unidade) {
        $this->db->select(' i.internacao_leito_id as leito_id,
                            il.nome as leito,
                            ie.nome as enfermaria,
                            p.nome as paciente,
                            p.paciente_id');
        $this->db->from('tb_internacao_leito il, tb_internacao_enfermaria ie, tb_paciente p');
        $this->db->where('ie.internacao_enfermaria_id = il.enfermaria_id');
        $this->db->where('il.ativo', 'f');
...
//        $this->db->from('tb_paciente p');
    ...
    function transferirpacienteleito($novo_leito, $paciente_id){
        //atualizando o leito na tabela internacao
        $this->db->set('leito', $novo_leito);
        $this->db->where('paciente_id', $paciente_id);
        $this->db->update('tb_internacao');
        $erro = $this->db->_error_message();
                if (trim($erro) != "") { // erro de banco
